...
const encodeFilterQuery = (filter) => encodeURIComponent(filter).replace(/%20/g, "+")
const dependabotHighCriticalFilter = "is:open severity:critical severity:high"
const codeqlHighCriticalFilter = "is:open tool:CodeQL severity:critical severity:high"
const dependabotAlertsUrl =
  `${context.serverUrl}/${owner}/${repo}/security/dependabot` + `?q=${encodeFilterQuery(dependabotHighCriticalFilter)}`
const codeqlAlertsUrl =
  `${context.serverUrl}/${owner}/${repo}/security/code-scanning` + `?q=${encodeFilterQuery(codeqlHighCriticalFilter)}`

const normalizeSeverity = (value) => (value || "").toString().trim().toLowerCase()

const emptySeverityCounts = () => Object.fromEntries(SEVERITY_COLUMNS.map((s) => [s, 0]))
...
const rowTotal = (counts) => SEVERITY_COLUMNS.reduce((sum, severity) => sum + counts[severity], 0)
...
const isCodeqlAlert = (alert) => {
  const toolName = (alert.tool?.name || alert.tool_name || "").toString().toLowerCase()
...
try {
  ...
  core.warning("Ensure the SAST GitHub App has Dependabot alerts (Read) on this repository.")
}

try {
  const scanningAlerts = await github.paginate(github.rest.codeScanning.listAlertsForRepo, {
    owner,
    repo,
    state: "open",
    per_page: 100
  ...
  errors.push(message)
  core.warning(message)
  core.warning("Ensure the SAST GitHub App has Code scanning alerts (Read) or security-events access.")
}

const dependabotCounts = countBySeverity(dependabotAlerts, (alert) => alert.security_advisory?.severity)
...
core.summary.addRaw(`Open critical/high alerts for **${repositoryName}**: **${totalOpenAlerts}**`)
